...
async def login_page(request: Request, error: Optional[str] = None):
    ...
    admin_password = os.environ.get("DEFAULT_ADMIN_PASSWORD", "12345")
...
async def login(response: Response,
    ...
    if not user:
        return RedirectResponse(url="/login?error=Invalid+email+or+password",
                                status_code=303)
    ...
    redirect_response.set_cookie(
        ...
        max_age=int(
            access_token_expires.total_seconds()),  # Cookie expiry in seconds
        expires=int(
            access_token_expires.total_seconds()),  # For older browsers
